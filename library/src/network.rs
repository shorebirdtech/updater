...
pub fn send_patch_check_request(
    ...
    let response = config
        .network_hooks
        .make_patch_check_request(url, request)?;
...
pub fn report_successful_patch_install(
    ...
    config
        .network_hooks
        .report_patch_install_success(url, request)
}
...
pub fn download_to_path(
    ...
    let mut bytes = network_hooks.download_file(url)?;
...
mod tests {
    ...
    // TODO(bryanoltman): fix once network client trait is working
    // #[test]

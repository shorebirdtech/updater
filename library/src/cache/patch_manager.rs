...
mod add_patch_tests {
    ...
    // Signature verification tests - these verify that signature checking happens at install time.
    ...
    fn errs_if_public_key_is_invalid() {
        ...
        // Should fail at install time because the public key is invalid
    ...
    fn errs_if_signature_is_missing_when_public_key_configured() {
        ...
        // Should fail at install time because signature is missing
...
mod validate_next_boot_patch_tests {
    ...
    // Note: Signature verification tests have been moved to add_patch_tests since
    // signature verification now happens at install time, not boot time.
    // Boot-time validation only checks file existence and size.
    ...
    fn succeeds_if_patch_has_valid_signature() -> Result<()> {
        ...
        // Boot-time validation should succeed (only checks file existence and size)
        assert!(manager.next_boot_patch().is_some());
        assert!(manager.validate_next_boot_patch().is_ok());
        assert!(manager.next_boot_patch().is_some());
